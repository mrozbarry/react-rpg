...
export default createClass({
  ...
  _handleMapChange (nextProps) {
    this.buffers = {
      ...
      characters: this._createCanvasBufferForMap(nextProps.map),
  ...
  render () {
    ...
    return (
      <Layer name="map" className="map">
        <canvas className="map__composite" ref={(self) => this.compositer = self} width={windowSize[0]} height={windowSize[1]}>
          Your platform does not support canvas. That's probably the least of your problems, while trying to run this game.
        </canvas>
        <img src="/assets/images/Hitman 1/hitman1_gun.png" width="32" height="32" style={{ position: "absolute", top: 0, left: "32px" }} />
      </Layer>
    )
  }
})
